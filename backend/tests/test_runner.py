...
def test_run_captures_stdout():
    ...
    outcome = run_one(code, "python", "solve", {"x": 5})
...
def test_run_captures_stderr():
    ...
    outcome = run_one(code, "python", "solve", {})
...
def test_run_respects_timeout():
    ...
    outcome = run_one(code, "python", "solve", {}, timeout_s=1)
    assert outcome.error is not None
...
def test_run_truncates_stdout_over_cap():
    ...
    outcome = run_one(code, "python", "solve", {}, output_cap_bytes=64 * 1024)
...
def test_run_class_based_ops():
    ...
    outcome = run_one(code, "python", "Counter", input_data)
...
def test_run_javascript_basic():
    ...
    outcome = run_one(code, "javascript", "solve", {"a": 2, "b": 3})
